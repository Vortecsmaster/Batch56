...
module Batch56CustomTyped where
...
PlutusTx.unstableMakeIsData ''MyWonderfullRedeemer   -- makeIsDataIndexed
...
typedValidator = Scripts.mkTypedValidator @Typed                  -- Tell the compiler that you are using Typed
    $$(PlutusTx.compile [|| customTypedRedeemer ||]) 
    $$(PlutusTx.compile [|| wrap ||])                             -- Provide the translation into high level typed to low level typed
  where
    wrap = Scripts.wrapValidator @() @MyWonderfullRedeemer        -- Tell wrapvalidator which types to use for Datum and Redeemer
